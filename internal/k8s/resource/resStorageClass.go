...
func (r *CephRbd) SetMonitors(monitors string) error {
	...
	if len(monis) <= 0 {
		return internal.NewError("monitor is empty")
	}
	for _, v := range monis {
		// 检查ip:port格式
		url := strings.Split(v, ":")
		if !utils.IsIP(url[0]) {
			return internal.NewError("ip is empty")
		}
		// 端口检查
		if len(url) <= 1 || url[1] == "" || utils.ToInt(url[1]) <= 0 {
			return internal.NewError("port is empty")
		}
	}
	r.Monitors = monitors
	return nil
...
func (r *CephRbd) SetData(data []map[string]string) error {
	if len(data) <= 0 {
		return internal.NewError("no data to set")
	}
	for _, v := range data {
		if v["val"] == "" {
			return internal.NewError(v["key"] + " is empty")
		}
		switch v["key"] {
		case "monitors":
			monitors := strings.Split(v["val"], ",")
			if err := r.SetMonitors(strings.Join(monitors, ",")); err != nil {
				return internal.NewError(v["key"] + " format error")
			}
		case "adminId":
			r.SetAdminId(v["val"])
