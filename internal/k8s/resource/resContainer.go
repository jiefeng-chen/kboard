...
func (r *Container) SetImagePullPolicy(imgplc string) error {
	if imgplc == "" {
		return internal.NewError("image pull policy is empty")
	}
...
func (r *Container) SetCommands(cmds []string) error {
	if len(cmds) <= 0 {
		return internal.NewError("commands is empty")
	}
...
func (r *Container) SetWorkDir(wkdir string) error {
	if wkdir == "" {
		return internal.NewError("work directory is empty")
	}
	r.WorkingDir = wkdir
	return nil
}
...
func NewResource() *ContainerResources {
	return &ContainerResources{
		Limits:   internal.Limits{Cpu: "", Memory: ""},
		Requests: internal.Request{Cpu: "", Memory: ""},
	}
}
