...
type IResDaemonSet interface {
	...
	SetVolume(*internal.Volume) error
...
func (r *ResDaemonSet) SetMetaDataName(name string) error {
	if name == "" {
		return internal.NewError("metadata name is empty")
	}
	// 设置 .metadata.name
...
func (r *ResDaemonSet) SetNamespace(ns string) error {
	if ns == "" {
		return internal.NewError("metadata namespace is empty")
	}
...
func (r *ResDaemonSet) SetMatchLabels(labels []map[string]string) error {
	if len(labels) > 0 {
		for _, v := range labels {
			for key, val := range v {
				if key == "" {
					return internal.NewError("matchLabels key is empty")
				}
...
func (r *ResDaemonSet) SetTolerations(tolers []map[string]string) error {
	if len(tolers) > 0 {
		for _, v := range tolers {
			if v["key"] == "" {
				return internal.NewError("toleration key is empty")
			}
...
func (r *ResDaemonSet) AddContainer(container internal.IContainer) error {
	if container == nil {
...
func (r *ResDaemonSet) SetTerminationGracePeriodSeconds(second string) error {
	if second == "" {
		return internal.NewError("termination grace period seconds is empty")
	}
...
func (r *ResDaemonSet) SetVolume(vol *internal.Volume) error {
	if vol == nil {
...
func (r *ResDaemonSet) SetNodeSelector(selectors []map[string]string) error {
	if len(selectors) > 0 {
		for _, v := range selectors {
			if v["key"] == "" {
				return internal.NewError("node selector's key is empty")
			}
